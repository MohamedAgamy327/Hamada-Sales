...
{
    public class SupplyOfferUpdateViewModel : ValidatableBindableBase
    {
        ...
        private async void ExecuteAddToBillAsync()
        {
            ...
            SelectedSupplyOffer.Cost = SupplyOfferCategories.Sum(s => s.CostTotal);
            SelectedSupplyOffer.CostAfterTax = SupplyOfferCategories.Sum(s => s.CostTotalAfterTax);
            SelectedSupplyOffer.TotalDiscount = SupplyOfferCategories.Sum(s => s.DiscountValueTotal);
        ...
        private void EditMethod()
        {
            ...
            SelectedSupplyOffer.CostAfterTax = SupplyOfferCategories.Sum(s => s.CostTotalAfterTax);
            SelectedSupplyOffer.TotalDiscount = SupplyOfferCategories.Sum(s => s.DiscountValueTotal);
            OldCosts = new ObservableCollection<SupplyCategory>(_supplyCategoryServ.GetOldCosts(_newSupplyOfferCategory.CategoryID, _selectedSupplyOffer.ClientID));
        }
        ...
        private bool CanExecuteSave()
        {
            try
            {
                if (SelectedSupplyOffer.HasErrors)
                    return false;
                else
                    return true;
            }
